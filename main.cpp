...
void TestUpdate(
        ...
        const vector<string> &queries,
        const vector<string> &expected,
        const vector<string> &updated_docs,
        const vector<string> &updated_queries,
        const vector<string> &expected_after_update
) {
    istringstream docs_input(Join('\n', docs));
    istringstream queries_input(Join('\n', queries));
    istringstream docs_updated(Join('\n', updated_docs));
    istringstream queries_updated(Join('\n', updated_queries));
    ...
    srv.UpdateDocumentBase(docs_updated);
    ostringstream updated_output;
    srv.AddQueriesStream(queries_updated, updated_output);
    const string updated_result = updated_output.str();
    const auto updated_lines = SplitBy(Strip(updated_result), '\n');
    ASSERT_EQUAL(updated_lines.size(), expected_after_update.size());
    for (size_t i = 0; i < updated_lines.size(); ++i) {
        ASSERT_EQUAL(updated_lines[i], expected_after_update[i]);
    }
...
void TestConsistency() {
    ...
    const vector<string> docs_updated = {
            "milk a",
            "milk b",
            "milk c",
            "milk d",
    ...
    const vector<string> queries_updated = {"milk", "water", "rock"};
    const vector<string> expected_after_update = {
            Join(' ', vector{
                    "milk:",
                    "{docid: 0, hitcount: 1}",
...
int main() {
    ...
    RUN_TEST(tr, TestNine);
    RUN_TEST(tr, TestConsistency);
}
